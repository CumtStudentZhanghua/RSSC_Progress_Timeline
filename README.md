...
| 2022.03 | **RS** | MopNet-GCN-ResNet50 | [Multi-Output Network Combining GNN and CNN for Remote Sensing Scene Classification](https://www.mdpi.com/2072-4292/14/6/1478) | ResNet50, GAT, and GCN | Use unsupervised superpixel segmentation on raw images to construct undirected graph (nodes=superpixel regions, edges=spatial adjacency); use intermediate CNN feature map to initialize node features; employ GCN/GAT to aggregate local node features into global graph representation via weight-and-sum pooling; train dual-branch (CNN + GNN) with joint loss, infer using GNN output | [N/A] | ✅ |
| 2022.03 | **TGRS** | SCViT | [SCViT: A Spatial-Channel Feature Preserving Vision Transformer for Remote Sensing Image Scene Classification](https://ieeexplore.ieee.org/document/9729845) | ViT |  | Use PA module to iteratively aggregate local tokens via reshape-split-connect with MSA+MLP, add cls token and linear embedding, ViT encoder for global modeling, LCA module for channel weighting in classification token | ✅ |
| 2022.04 | **GRSL** | MLF2Net_SAGM | [Multilayer Feature Fusion Network With Spatial Attention and Gated Mechanism for Remote Sensing Scene Classification](https://ieeexplore.ieee.org/document/9770786) | ResNet50 | Extract f2, f3, and f4 from ResNet50, apply RSAM(Residual Spatial Attention Module) for spatial enhancement per layer, use GFM(Gated Fusion Module) to compute softmax gates weights p/q/l, and obtain a final feature by weighted sum | [N/A] | ✅ |
| 2022.04 | **RS** | ACGLNet | [An Attention Cascade Global–Local Network for Remote Sensing Scene Classification](https://www.mdpi.com/2072-4292/14/9/2042) | ResNet50 and DenseNet121 | Extract {C₂–C₅} from ResNet50+DenseNet121, use C₅ spatial attention to weight lower layers → Fₗ, apply cascaded top-down fusion → Pₗ → average → Nₘ→GAP → Nₘ, dual-model fusion via Count-Sketch+FFT bilinear pooling | [N/A] | ✅ |
| 2022.05 | **GRSL** | MITformer | [MITformer: A Multiinstance Vision Transformer for Remote Sensing Scene Classification](https://ieeexplore.ieee.org/abstract/document/9777975) | ViT | Treat scene as bag and patches as instances; ViT backbone with AMLP (FC-attention weighted MLP) per layer to enhance local dims; dual predictions: p₁ from class token (LN), p₂ from patch tokens processed by MIL aggregation (linear+ReLU+spatial attention→weighted sum); fuse p₁ and p₂ by average → final Ŷ; train with cross-entropy + semantic consistency loss (L1 between p₁ and p₂) | [N/A] | ✅ |
...
| 2022.07 | **TGRS** | EMTCAL | [EMTCAL: Efficient Multiscale Transformer and Cross-Level Attention Learning for Remote Sensing Scene Classification](https://ieeexplore.ieee.org/document/9844016) | XXX | XXX | [🔗 Code](https://github.com/TangXu-Group/Remote-Sensing-Images-Classification/tree/main/EMTCAL) | ❌ |
