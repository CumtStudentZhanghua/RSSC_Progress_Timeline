...
| 2022.05 | **GRSL** | MITformer | [MITformer: A Multiinstance Vision Transformer for Remote Sensing Scene Classification](https://ieeexplore.ieee.org/abstract/document/9777975) | ViT | Treat scene as bag and patches as instances; ViT backbone with AMLP (FC-attention weighted MLP) per layer to enhance local dims; dual predictions: p₁ from class token (LN), p₂ from MIL aggregation (linear+ReLU+spatial attention→weighted sum); fuse p₁ and p₂ by average → final Ŷ; train with cross-entropy + semantic consistency loss (L1 between p₁ and p₂) | [N/A] | ✅ |
